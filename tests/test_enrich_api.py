...
def test_enrich_rejects_empty_targets(client):
    resp = client.post("/api/enrich", headers=auth(client), json={"targets": []})
    assert resp.status_code == 422


def test_enrich_rejects_target_without_name_or_domain(client):
    resp = client.post(
        "/api/enrich", headers=auth(client), json={"targets": [{"company_name": "   "}]}
    )
...
def test_job_runs_to_completion_and_is_pollable(client):
    ...
    final = wait_for_job(client, headers, f"/api/enrich/jobs/{job_id}")
...
def test_unknown_job_returns_404(client):
    assert client.get("/api/enrich/jobs/not-a-real-id", headers=auth(client)).status_code == 404
...
def test_rejects_when_no_source_enabled(client, db_path):
    settings = AppSettings()
    ...
    resp = client.post(
        "/api/enrich", headers=auth(client), json={"targets": [{"company_name": "Acme"}]}
    )
